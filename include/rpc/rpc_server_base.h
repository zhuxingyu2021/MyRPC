...
namespace MyRPC{
    ...
    public:
        ...
        void SetError(CTXType& context, Common::Errortype err){
            static_cast<Derived*>(*this)->SetError(context, err);
        }
        ...
        std::function<Common::Errortype()>* FindService(const std::string& service_name){
            std::unique_lock<SpinLock> lock(m_service_table_lock);
    ...
    protected:
        // 服务表，用于记录及查询服务对应的函数
        std::unordered_map<std::string, std::function<Common::Errortype()>> m_service_table;
